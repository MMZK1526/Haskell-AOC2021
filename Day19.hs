...
scannerPs pz = findScannerPos (S.singleton 0) (M.singleton 0 (id, [0, 0, 0]))
  where
    cmp ps ps'
      ...
      $ fmap ( second (maximum . fmap swap . C.group . liftM2 (zipWith (-)) ps) 
             . ap (,) (`map` ps')
             ) rotations
    scannerRPos
    ...
    findScannerPos frontier known = go fs known scannerRPos
      where
        ...
        go fs k (((i, j), (t, pos)) : ps)
          ...
          where
            k'         = M.insert j (tI . t, zipWith (+) posI (tI pos)) k
            (tI, posI) = k M.! i
...
day19Part1 scanners = length . S.fromList . concat . zipWith merge [0..]
  where
    merge i = let (t, oft) = scanners M.! i in map (flip (zipWith (+)) oft . t)
