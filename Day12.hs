...
day12Part1 = go "start" . filter (("start" /=) . snd)
  where
    go "end" _     = 1
    go start paths = sum $ ap go filterPath <$> nexts
      where
        nexts = snd <$> filter ((== start) . fst) paths
        filterPath next
...
day12Part2 = go True S.empty "start" . filter (("start" /=) . snd)
  where
    ...
    go canGoBack set start paths
      | isUpper (head start) = sum $ flip (go canGoBack set) paths <$> nexts
      | start `notElem` set  = sum $ flip (go canGoBack set') paths <$> nexts
      | canGoBack            = sum $ flip (go False set') paths <$> nexts
      | otherwise            = 0
      where
